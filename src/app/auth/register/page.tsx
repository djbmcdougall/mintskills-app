...
export default function RegisterPage() {
  ...
  async function handleGitHub() {
    // Role stored in metadata after OAuth completes via profile update
    const supabase = createClient()
      ...
      <div className="w-full max-w-sm">
        ...
        {sent ? (
          <div className="mt-10 bg-surface border border-border p-6 text-center">
            ...
            <p className="text-text-3 text-scale-sm font-mono mt-2">Click the link in the email to continue.</p>
          </div>
        ) : (
          <form onSubmit={handleMagicLink} className="mt-8 flex flex-col gap-3">
            {/* Role selector */}
            <div>
              <p className="text-text-3 text-scale-xs font-mono mb-2">I am here to…</p>
              <div className="flex gap-1">
                {ROLES.map(r => (
                  <button
                    ...
                    className={`flex-1 py-2 text-scale-xs font-mono border transition-colors ${
                      role === r.value
